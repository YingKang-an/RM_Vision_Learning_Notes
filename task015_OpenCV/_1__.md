我们将从最简单的开始，一步一步来。请确保你已经安装了OpenCV并配置好了你的C++项目。
这里假设你已经有一个OpenCV的环境，如果没有，请先安装OpenCV并配置到你的项目中。
...
注意：所有绘制操作都会修改原始图像，因此我们通常在一个空白图像或已有图像上绘制。

我们先创建一个空白的图像作为画布，然后在其上绘制。
