...
## 🛠️ 编译环境要求
- 编译器：GCC 8+ / Clang 6+ / MSVC 2019+（需开启 C++17/20/23 标准）
- 编译命令示例（GCC）：`g++ -std=c++20 文件名.cpp -o 可执行文件 -pthread`（并发模块需加 `-pthread`）
- 调试工具：建议使用 GDB/LLDB 或 IDE（CLion/VS2022）调试，理解内存布局与执行流程
