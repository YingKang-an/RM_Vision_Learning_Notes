...
for i in range(9):      # 行
    for j in range(6):  # 列
        # Z_w = 0（棋盘平面）
        points_3d.append([j*30, i*30, 0])  # 单位：mm
```
...
#### 3.4.3 畸变矫正流程
```python
def undistort_points(points_distorted, K, dist_coeffs):
    """
    畸变矫正核心算法
    输入：points_distorted - 畸变点[N,2]
          K - 内参矩阵[3,3]
          dist_coeffs - 畸变系数[k1,k2,p1,p2,k3,...]
    输出：points_undistorted - 矫正点[N,2]
    """
    # 1. 归一化到图像平面
    fx, fy = K[0,0], K[1,1]
    cx, cy = K[0,2], K[1,2]
    
    x_norm = (points_distorted[:,0] - cx) / fx
    y_norm = (points_distorted[:,1] - cy) / fy
    ...
    # 3. 径向畸变系数
    k1, k2, k3 = dist_coeffs[0], dist_coeffs[1], dist_coeffs[4]
    radial_correction = 1 + k1*r2 + k2*r4 + k3*r6
    
    # 4. 切向畸变系数
    p1, p2 = dist_coeffs[2], dist_coeffs[3]
    x_tang = 2*p1*x_norm*y_norm + p2*(r2 + 2*x_norm**2)
    y_tang = p1*(r2 + 2*y_norm**2) + 2*p2*x_norm*y_norm
    
    # 5. 应用矫正
    x_corrected = x_norm * radial_correction + x_tang
    y_corrected = y_norm * radial_correction + y_tang
    
    # 6. 返回到像素坐标
    points_undistorted = np.column_stack([
        x_corrected * fx + cx,
        y_corrected * fy + cy
    ])
...
> 提示：本笔记为学习参考用途，实际应用时请结合具体硬件和场景进行调整。标定精度不仅取决于算法，更取决于数据质量和操作规范性。建议在实践中建立标准操作流程(SOP)以确保结果一致性。

---

这份重新梳理的笔记力求**详尽、系统、实用**，从理论基础到工程实践，从经典算法到前沿研究，涵盖了张正友相机标定法的方方面面。总字数约4.5万字，包含大量代码示例、数学推导和实际建议，适合作为深入学习和工作参考的资料。