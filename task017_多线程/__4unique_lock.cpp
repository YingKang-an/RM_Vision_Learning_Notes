...
// 因为 mutex 在管理方面有瑕疵,因此出现了一个互斥量封装器lock_guard来智能地管理mutex.
// 而lock_guard只是简单的管理,功能比较弱,有瑕疵.因此需要搞出来一个功能更强大的东西出来.而这个东西就是unique_lock.
// 本质上来说lock_guard 和unique_lock都是为了更好地使用各种锁而诞生的.但是unique_lock更为灵活,功能更强大,可以做的操作比较多.
