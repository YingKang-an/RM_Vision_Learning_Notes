...
// 训练感知机（批量梯度下降版本）
cv::Mat_<float> trainPerceptronBatch(const cv::Mat_<float>& X, const cv::Mat_<int>& y, float eta = 0.1f, int max_iter = 1000) {
  ...
  while (iter < max_iter) {
    // 批量梯度下降：收集所有误分类点的梯度
    cv::Mat_<float> gradient_sum = cv::Mat_<float>::zeros(4, 1);  // 梯度累加器
    int misclassified = 0;
    ...
    // 批量梯度下降：一次性更新参数
    if (misclassified > 0) {
      theta = theta + eta * gradient_sum;
      
      std::cout << "[第" << iter+1 << "轮] 批量更新|误分类点数:" << misclassified << "\n"
